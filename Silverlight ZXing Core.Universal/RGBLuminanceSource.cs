...
public class RGBLuminanceSource : LuminanceSource
{
    ...
    public RGBLuminanceSource(int[] Pixels, int W, int H)
        : base(W, H)
    {
        int width = __width = W;
        ...
        //if (format == PixelFormat.Format8bppIndexed)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    int srcPixel = Pixels[x + (width * y)];
                    //Marauderz : My bitshift math is off, but something tells me I'm doing some extra unnecessary shifting
